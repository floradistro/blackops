...
import { loadToolRegistry, getToolMetadata, ToolRegistryEntry, ClientToolMetadata, invalidateToolCache } from "./services/tool-registry.js";
import { executeTool, ToolErrorType } from "./tools/executor.js";
import { AgentConfig, ClientMessage, ServerMessage, ConversationMeta } from "./types.js";
import {
  createConversation,
  getConversation,
...
function registryToAnthropicTools(tools: ToolRegistryEntry[]): Anthropic.Tool[] {
  return tools.map(t => {
  ...
  for (const msg of messages) {
    if (typeof msg.content === "string") {
      ...
      for (const block of msg.content) {
        if ("text" in block) chars += block.text?.length || 0;
        if ("content" in block) chars += String(block.content).length;
      }
    }
  }
    ...
    while (turn < maxTurns) {
      ...
      try {
        ...
        if (streamError.status === 429) {
          ...
          send(ws, { type: "error", error: "Rate limited. Please retry in a moment.", recoverable: true } as any);
          return;
        }
        if (streamError.status === 529) {
          ...
          send(ws, { type: "error", error: "Anthropic servers overloaded. Please retry.", recoverable: true } as any);
          return;
        }
        throw streamError;
      }
      ...
      const usage = response.usage as any;  // Extended usage type
      ...
      if (response.stop_reason === "tool_use") {
        ...
        for (const block of assistantContent) {
          if (block.type === "tool_use") {
            ...
            send(ws, { type: "tool_start", tool: block.name, input: block.input as Record<string, unknown> });

            // Execute tool with timeout (Anthropic best practice: prevent blocking)
            let result;
            try {
              const timeoutPromise = new Promise<never>((_, reject) =>
                setTimeout(() => reject(new Error(`Tool execution timeout after ${TOOL_EXECUTION_TIMEOUT_MS}ms`)), TOOL_EXECUTION_TIMEOUT_MS)
              );
